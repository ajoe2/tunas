...
def test_b2_host() -> None:
    ...
    meets, _ = parse_lines([A0, B1, b2, C1, d0(), Z0])
    host = meets[0].host
...
def test_c1_unattached() -> None:
    ...
    meets, _ = parse_lines([A0, B1, c1u, d0(), Z0])
    assert meets[0].clubs == []
    assert meets[0].individual_swims[0].club is None
...
def test_c2_team_entry() -> None:
    ...
    meets, _ = parse_lines([A0, B1, C1, c2, d0(), Z0])
    club = meets[0].clubs[0]
...
def test_d0_individual_swim() -> None:
    meets, _ = parse_lines([A0, B1, C1, d0(prelim="1:01.00", prelim_course="Y"), Z0])
    swims = meets[0].swimmers[0].individual_swims
...
def test_d0_missing_birthday_kept() -> None:
    ...
    assert report.warnings_for(record_type="D0", field="birthday")
    assert report.records_skipped == 0


def test_d0_missing_date_kept() -> None:
    meets, report = parse_lines([A0, B1, C1, d0(date=""), Z0])
    assert meets[0].individual_swims[0].date is None
    assert report.warnings_for(record_type="D0", field="date")
...
def test_d0_id_long_fallback() -> None:
    ...
    meets, report = parse_lines([A0, B1, C1, d0(uss=""), d3, Z0])
    sw = meets[0].swimmers[0]
...
def test_d0_outcome_codes() -> None:
    meets, report = parse_lines([A0, B1, C1, d0(finals="NS"), Z0])
...
def test_d0_course_x_dq_keeps_time() -> None:
    meets, _ = parse_lines([A0, B1, C1, d0(finals="1:00.00", finals_course="X"), Z0])
    res = meets[0].swimmers[0].individual_swims[0]
...
def test_d0_missing_course_present_time() -> None:
    ...
    course_warnings = [w for w in report.warnings if w.field == "course" and w.mandatory == "*"]
...
def test_d1_d2_contact_and_registration() -> None:
    ...
    meets, _ = parse_lines([A0, B1, C1, d0(), d1, d2, Z0])
    sw = meets[0].swimmers[0]
...
def test_d3_long_id_and_demographics() -> None:
    ...
    meets, _ = parse_lines([A0, B1, C1, d0(), d3, Z0])
    sw = meets[0].swimmers[0]
    assert sw.id_long == "49AC52F6961843"
...
def test_e0_relay_result() -> None:
    meets, _ = parse_lines([A0, B1, C1, e0(), f0(), Z0])
...
def test_e0_relay_dq() -> None:
    meets, _ = parse_lines([A0, B1, C1, e0(finals="1:45.00", finals_course="X"), f0(), Z0])
    assert meets[0].relays[0].status is ResultStatus.DQ
...
def test_f0_alternate_routed_and_excluded_from_swims() -> None:
    ...
    meets, _ = parse_lines([A0, B1, C1, e0(), alt, Z0])
    relay = meets[0].relays[0]
    assert relay.legs == []
...
def test_f0_missing_id_keeps_leg_with_none_swimmer() -> None:
    leg = f0(uss="", id_long="")
    meets, report = parse_lines([A0, B1, C1, e0(), leg, Z0])
    relay = meets[0].relays[0]
    assert len(relay.legs) == 1
    assert relay.legs[0].swimmer is None
    assert report.warnings_for(record_type="F0", field="id_short")


def test_f0_new_swimmer_from_relay() -> None:
    leg = f0(name="New, Swimmer", uss="NEW000000001")
    meets, _ = parse_lines([A0, B1, C1, e0(), leg, Z0])
    assert any(s.id_short == "NEW000000001" for s in meets[0].swimmers)
...
def test_relay_per_session_and_split_routing() -> None:
    ...
    meets, _ = parse_lines([A0, B1, C1, e0pf, f0pf, Z0])
    relays = meets[0].relays
    assert {r.session for r in relays} == {Session.PRELIMS, Session.FINALS}
    sw = next(s for s in meets[0].swimmers if s.relay_swims)
    assert len(sw.relay_swims) == 2  # one RelaySwim per session swum


def test_g0_splits_after_d0() -> None:
    meets, _ = parse_lines([A0, B1, C1, d0(), g0(), Z0])
    finals = next(s for s in meets[0].individual_swims if s.session is Session.FINALS)
...
def test_g0_multi_record_ordered_by_sequence() -> None:
    ...
    meets, _ = parse_lines([A0, B1, C1, d0(), g1, g2, Z0])
    finals = next(s for s in meets[0].individual_swims if s.session is Session.FINALS)
...
def test_g0_unparseable_split_kept_as_none() -> None:
    ...
    meets, report = parse_lines([A0, B1, C1, d0(), gr, Z0])
    finals = next(s for s in meets[0].individual_swims if s.session is Session.FINALS)
    assert finals.splits[0].time is not None
    assert finals.splits[1].time is None
    assert report.warnings_for(record_type="G0", field="split_time")


def test_z0_count_mismatch() -> None:
    z0_bad = rec((1, "Z0"), (3, "1"), (12, "02"), (58, "999"))
    meets, report = parse_lines([A0, B1, C1, d0(), z0_bad])
    from tunas import IssueKind

    assert report.warnings_for(kind=IssueKind.COUNT_MISMATCH)
    assert report.fields_recovered == 0  # COUNT_MISMATCH excluded


def test_unknown_record_type() -> None:
    j0 = rec((1, "J0"), (12, "qualifying time"))
    meets, report = parse_lines([A0, B1, j0, C1, d0(), Z0])
    from tunas import IssueKind

    assert report.warnings_for(kind=IssueKind.UNKNOWN_RECORD)
    assert len(meets[0].swimmers) == 1  # parse continues
